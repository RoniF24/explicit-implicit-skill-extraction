...
def _classify_mode(mode: str) -> Optional[str]:
    """
    Robust mapping for overall_metrics.csv 'mode' field.

    IMPORTANT: Any must be checked BEFORE Explicit because some strings look like '0.5_or_1.0'
    which contains '1.0' and would otherwise be misclassified as explicit.
    """
    m = _norm_mode(mode)
...
def _mode_priority(mode: str, key: str) -> int:
    """
    If multiple rows map to the same key, keep the most specific one.
    Bigger = more specific.
    """
    m = _norm_mode(mode)
...
def _plot_barh_with_zero_visible(ax, items: List[Tuple[str, float]], title: str, missing_msg: str) -> None:
    ...
    # Make zeros visible by plotting a tiny epsilon, but annotate real value
    eps = 0.01
    ...
    # Annotate real values (including 0.00)
...
def make_zero_shot_dashboard(run_dir: Path, out_name: str) -> Path:
    ...
    _plot_barh_with_zero_visible(
        ax_imp_bot,
        bot_imp,
        "Bottom 5 skills by F1 (Implicit ==0.5)",
        "Missing per_skill__implicit_only__f1.csv",
    )
